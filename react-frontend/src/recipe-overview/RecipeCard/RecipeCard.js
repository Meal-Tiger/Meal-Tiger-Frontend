	...
	if (props.images.length > 0) {
		return (
			<div onClick={props.onClick} className={styles['recipe-card']}>
				{/* Rezeptbild */}
				<div title={props.title} className={`${styles['recipe-card-img']}`} style={{backgroundImage: `url(${getImageUrl(props.images[0])}`}}></div>
				{/* Infobox */}
				<div className={styles['recipe-card-infobox']}>
					...
					<div className={` ${styles['recipe-card-info']} ${styles.user}`}>
						{/* User */}
						<div className={` ${styles.user}`} style={{backgroundImage: `url(${getImageUrl(props.user.profilePictureId)}`}}></div> <span>{props.user.username}</span>
					</div>
